...
module.exports = {
  signUp: catchAsyncErrors(async (req, res, next) => {
    ...
    await UserModel.findOne({ email }).exec(async (err, user) => {
      ...
      if (user) {
        return next(
          new ErrorHandler(`${user.email} is already registered`, 401)
        );
      }
  ...
  deleteUser: catchAsyncErrors(async (req, res, next) => {
    const { id } = req.params;
    await UserModel.findByIdAndDelete({ _id: id }).exec(async (err, result) => {
      if (err) {
        console.log(err);
        return next(new ErrorHandler("something went wrong", 401));
        ...
        await cloudDelete(result.avatar.public_id)
          .then(() => {
            res.status(200).json({
              success: 1,
